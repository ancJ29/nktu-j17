...
export function TransportDriverCell({ order }: { order: TransportOrder }) {
  ...
  if (order.isMultiTrip) {
    ...
    return (
      <Stack gap={2}>
        ...
        {trips.map((trip, i) => (
          <LegDriver key={i} index={i} trip={trip} plate={plateOf(trip.truckId)} />
        ))}
      </Stack>
    );
  }
  ...
  return (
    <Stack gap={2}>
      ...
      <Vehicle name={order.truckPlate} plate={plateOf(order.truckId)} />
    </Stack>
  );
}
...
function Vehicle({ name, plate }: { name?: string; plate?: string }) {
