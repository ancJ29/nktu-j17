...
const NKTU_FORM_COLS = { base: 1, sm: 2, md: 4 } as const;

export function NKTUSalesOrderForm({ variant }: { variant: SalesOrderFormVariant }) {
  ...
  const nktuCustomerSelectData = useMemo(
    () =>
      customers
      ...
      <Card withBorder radius="md" p="xl">
        ...
        >
          <Stack gap="md">
            ...
            {/* Individual-customer (Khách lẻ) toggle */}
            {hasCustomerRegistry && (
              <Switch
            ...
            {/* Row 1 (4-col): PO number · customer · delivery address · maps.
                The SO number is auto-generated + disabled, so NKTU hides it
                entirely instead of showing a read-only field. */}
            {(() => {
              ...
              return (
                <SimpleGrid cols={NKTU_FORM_COLS}>
                  <TextInput
            ...
            {/* Row 2 (4-col): assigned staff · delivery method · order + delivery dates */}
            <SimpleGrid cols={NKTU_FORM_COLS}>
              <EmployeeSelector
            ...
            {splitNotesCfg ? (
              <SimpleGrid cols={{ base: 1, md: 3 }}>
                {/* Notes — the general order note is always shown. NKTU adds
                department-scoped warehouse + driver notes (visibility-gated)
                alongside it; other clients keep just the general note. */}
                <Textarea
