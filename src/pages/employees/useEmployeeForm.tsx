...
const hasBulkImport = hasBulkImportForEmployees() && supportsBulkImport;
...
export function useEmployeeForm(): EmployeeFormParts {
    ...
    async (values: EmployeeFormValues) => {
      ...
      try {
        ...
        if (isEdit && id) {
          ...
          const res = await createEmployee({ ...rest, email, extra });
  ...
  const handleBulkUpload = useCallback(async () => {
    ...
    try {
      ...
      const res = await importBatchEmployees(items);
  ...
  if (fetching) return { ready: false };
  ...
  return {
    ready: true,
    isEdit,
    pageTitle: isEdit ? t('employees.editEmployee') : t('employees.addEmployee'),
    single: singleBody,
    bulk: bulkBody,
    onCancel: navigateToList,
  };
