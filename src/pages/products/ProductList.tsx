...
import { buildProductColumnFilterDefs } from './productColumnFilters';
import { ProductDataTable } from './ProductDataTable';
import type { ProductListVariant } from './productListVariant';
...
type ProductFilterDimensions = Pick<
  ProductFilters,
  'status' | 'category' | 'inventoryDisplay' | 'columnFilters'
>;
...
type ProductListProps = {
  readonly variant: ProductListVariant;
};

export function ProductList({ variant }: ProductListProps) {
  ...
  const filters = useMemo(
    () => ({
      ...
      inventoryDisplay: inventoryDisplayFilter,

      columnFilters: filterState.columnFilters,
    }),
    [filter, categoryFilter, inventoryDisplayFilter, filterState.columnFilters],
  );
  ...
  const columnFilterDefs: ColumnFilterDef<(typeof allProducts)[number]>[] = useMemo(
    () =>
      variant.showColumnHeaderFilters
        ? buildProductColumnFilterDefs({
            onHandByCode,
            priceVisible,
            inventoryEnabled,
        ...
        : [],
    [variant.showColumnHeaderFilters, onHandByCode, columnFilterLabels],
  );

  const listedProducts = useMemo(
    () => allProducts.filter((p) => !p.extra?.isDeleted),
    [allProducts],
  );
  const columnFilters = useColumnFilters(listedProducts, columnFilterDefs, {
    value: columnFilterValues,
    onChange: setColumnFilterValues,
  });
  const columnFilterPredicate = columnFilters.predicate;
  ...
  const filterFn = useCallback(
    (item: (typeof allProducts)[number], f: ProductFilterDimensions) => {
      ...
      return columnFilterPredicate(item);
    },
