...
): (label: string) => Type10FeeColumn {
  const byKey = new Map<string, Type10FeeColumn>();
  for (const [column, values] of Object.entries(FEE_NAMES) as Array<
    [Type10FeeColumn, ReadonlyArray<string>]
  >) {
...
export const buildCustomerReportType10: CustomerReportBuilder = (
  orders,
  {
    seller,
    customer,
  ...
  const moocKeys = new Set([feeKey(MOOC_FEE), feeKey(resolveFeeName(MOOC_FEE))]);
  const isMooc = (label: string) =>
    moocKeys.has(feeKey(label)) || moocKeys.has(feeKey(resolveFeeName(label)));

  const readMoney = (o: TransportOrder) => {
    const service = new Map<Type10FeeColumn, number>();
    const chiHo: TransportOrderFee[] = [];
    let mooc = 0;
    let vatBase = 0;
    for (const fee of readFeeLines(o)) {
      ...
      if (isMooc(fee.label)) {
        mooc += amount;
      } else if (fee.kind === 'service') {
        const column = columnOf(fee.label);
        service.set(column, (service.get(column) ?? 0) + amount);
        if (fee.vatable) vatBase += amount;
      } else if (amount !== 0) {
        chiHo.push({ ...fee, label: resolveFeeName(fee.label) });
      }
    }
    const vat = roundVat(vatBase * (o.vatRate || 0), !!o.roundDown);
    const serviceSum = [...service.values()].reduce((a, b) => a + b, 0);
    return { service, vat, total: serviceSum + vat, mooc, chiHo };
  };

  const moneyOf = new Map(rows.map((o) => [o, readMoney(o)]));
  const groups = Math.max(MIN_CHI_HO_GROUPS, ...rows.map((o) => moneyOf.get(o)!.chiHo.length));
  ...
  const C_MOOC = C_TOTAL + 1;
  const C_CONTRACT = C_MOOC + 1;
  ...
  leaf(C_PICKUP_DATE, 'NGÀY LẤY');
  leaf(C_DROP_DATE, 'NGÀY HẠ');
  leaf(C_MOOC_DAYS, 'SỐ NGÀY LƯU MOOC');
  ...
  rows.forEach((o, i) => {
    ...
    const { requestedPickupDate, dropoffDate, moocStorageDays } = o.extra?.type5Specific ?? {};
    if (typeof requestedPickupDate === 'string' && requestedPickupDate) {
      row[C_PICKUP_DATE] = formatDate(requestedPickupDate);
    }
    if (typeof dropoffDate === 'string' && dropoffDate) row[C_DROP_DATE] = formatDate(dropoffDate);

    if (typeof moocStorageDays === 'number') row[C_MOOC_DAYS] = moocStorageDays;
    row[C_ORDER_NO] = o.extra?.customerOrderNumber ?? '';
    ...
    const money = moneyOf.get(o)!;
    SERVICE_COLUMNS.forEach(({ column }, k) =>
      putMoney(C_FEE0 + k, money.service.get(column) ?? 0),
    );
    putMoney(C_VAT, money.vat);
    putMoney(C_TOTAL, money.total);
    putMoney(C_MOOC, money.mooc);
    row[C_CONTRACT] = o.transportContractNo ?? '';
    money.chiHo.forEach((fee, g) => {
  ...
  const mooc = sums.get(C_MOOC) ?? 0;

  const summary = [
    { label: 'Số tiền cước vận chuyển:', value: service, note: '(Đã bao gồm VAT)' },
    { label: 'Số tiền chi hộ', value: chiHo, note: '(Đã bao gồm VAT)' },
    { label: 'Phí neo:', value: mooc, note: '' },
    { label: 'Tổng số tiền cần thanh toán:', value: service + chiHo + mooc, note: '' },
  ];
  const notes = TRANSFER_NOTES(seller.name);
  ...
  for (let i = 0; i < Math.max(notes.length, summary.length); i++) {
    ...
    if (note && note.value === undefined) {
      ...
      row[C_DATE] = note.text;
      row[C_TRUCK] = note.value!;
      merges.push({ s: { r, c: C_TRUCK }, e: { r, c: C_NOTE_END } });
    }
  ...
  ws['!cols'] = Array.from({ length: colCount }, (_, c) => {
    ...
    if (c === C_DATE || c === C_PICKUP_DATE || c === C_DROP_DATE) return { wch: 11 };
    if (c === C_MOOC_DAYS) return { wch: 9 };
    if (c === C_ORDER_NO || c === C_TRUCK) return { wch: 13 };
